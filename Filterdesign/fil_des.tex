...
\author{Ambati Krishna Kaustubh\\EE23BTECH11005}
\date{}
...
$\Omega_{p1} = 0.4898$, $\Omega_{p2} = 0.5913$ and $\Omega_{s1} = 0.4686$, $\Omega_{s2} = 0.6214$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.538$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1015$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.468\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.533\\
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.468.
\end{align}
...
\input{tables/fil_des_tab}
we get $N\geq 3.27$ and $0.315 \leq \epsilon \leq 0.61$\\
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/krishnakautubh11005/EE1205/tree/main/Filterdesign/codes/plot_1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot_1.png}
\caption{The Analog Low-Pass Frequency Response for $0.315  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/krishnakaustubh11005/EE1205/tree/main/Filterdesign/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/krishnakaustubh11005/EE1205/tree/main/Filterdesign/codes/pole_zero.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    \abs{ H_{a,LP}(s_L)} &= \frac{1}{\sqrt{1+\epsilon^2}} \text{at} \hspace{5p} \Omega_{L}=1 \implies s_{L} = j \label{eq:Gain_eq_LP} 
\end{align}
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{4.42\times 10^{-5}s^4}{s^8 + 0.132s^7 + 1.176s^6 + 0.115s^5 + 0.513s^4 + 0.033s^3 + 0.098s^2 + 0.0032s + 0.007} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{3.0772 - 13.1928z^{-1} + 32.8368z^{-2} - 52.6072z^{-3} + 60.828^{-4} - 49.9848z^{-5} + 29.6432z^{-6} - 11.3192z^{-7} + 2.5108z^{-8}}
\end{align}
where $G=4.42$    
\begin{figure}[H]
\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/FIR_1.png}
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.315n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.315n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
